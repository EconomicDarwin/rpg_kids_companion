...
- Plain HTML, CSS, vanilla JS. **No framework, no build step, no dependencies.**
  `app/` is the deployable site as-is. (The export in `tools/` is the one exception:
  it needs Pillow to downscale canon's print-resolution art. Nothing ships to a
  tablet that needed a build.)
...
or register the service worker). Verify: every hero tab renders, Treasure shows party
